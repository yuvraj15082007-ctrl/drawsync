...
function leaveRoom(socket, pin) {
    ...
    // Once everyone has left, destroy the room completely — its board, PIN,
    // and history all disappear. If someone types the same PIN later, they
    // get a brand-new empty room instead of walking into the old one.
    if (pin !== "public" && Object.keys(room.users).length === 0) {
        delete rooms[pin];
    }
}
...
io.on("connection", (socket) => {
    ...
    socket.on("draw", ({ pin, stroke }) => {
        const room = rooms[pin];
        if (!room) return;
        if (room.mode === "teaching" && socket.id !== room.teacherId && !room.permitted[socket.id]) {
            // Permission was likely revoked mid-stroke — tell the sender so their
            // local canvas can roll back instead of showing a stroke no one else has.
            socket.emit("drawRejected", { sid: stroke.sid });
            return;
        }
